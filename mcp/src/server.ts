...
async function resolveScope(projectIdOrHint: string | undefined): Promise<ScopeResolution> {
...
function sessionPromptTemplate(scope: ScopeResolution) {
...
async function preToolGovernance(name: ToolName, args: Record<string, unknown>) {
...
async function inferAndRecall(args: Record<string, unknown>) {
  const utterance = asString(args.utterance, "utterance");
  if (!utterance) throw badRequest("utterance is required");
...
function buildSynthesis(payload: Awaited<ReturnType<typeof inferAndRecall>>, utterance: string, buildContext?: string) {
  const top = payload.results.slice(0, 12).map((row: any) => ({
...
export async function startMcpServer(port = 4318) {
  ...
  app.post("/mcp/call", async (req, res) => {
    try {
      const body = req.body as { name?: ToolName; arguments?: Record<string, unknown> };
      const name = body.name;
      const args = body.arguments ?? {};
      if (!name || !toolSpecs.find((t) => t.name === name)) throw badRequest("Unknown tool name");
  ...
  app.post("/tools/recent_chatgpt", async (req, res) => {
    try {
      const result = await runTool("recent_chatgpt", (req.body as Record<string, unknown>) ?? {});
      res.json(result);
    } catch (err) {
      sendError(res, err);
    }
  ...
  app.post("/tools/recall_chatgpt", async (req, res) => {
    try {
      const result = await runTool("recall_chatgpt", (req.body as Record<string, unknown>) ?? {});
      res.json(result);
    } catch (err) {
      sendError(res, err);
    }
  ...
  app.post("/tools/infer_and_recall", async (req, res) => {
    try {
      const result = await runTool("infer_and_recall", (req.body as Record<string, unknown>) ?? {});
      res.json(result);
    } catch (err) {
      sendError(res, err);
    }
  ...
  app.post("/tools/conversational_memory_action", async (req, res) => {
    try {
      const result = await runTool("conversational_memory_action", (req.body as Record<string, unknown>) ?? {});
      res.json(result);
    } catch (err) {
      sendError(res, err);
    }
