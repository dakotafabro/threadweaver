...
program
  ...
  .action(async function (opts: { file: string; project?: string; access: string }) {
    await initPolicyStore();
    ...
    if (projectId) {
      const access = opts.access as "metadata" | "summary" | "markdown" | "ask";
...
connect
  ...
  .requiredOption("-f, --file <path>", "path to conversations export file or directory")
  .action(async function (opts: { file: string }) {
    await initPolicyStore();
...
projects
  ...
  .option("-a, --access <level>", "metadata|summary|markdown|ask", "summary")
  .action(async function (opts: { project: string; access: string }) {
    const access = opts.access as "metadata" | "summary" | "markdown" | "ask";
...
projects
  .command("deny")
  .requiredOption("-p, --project <id>", "project id")
  .action(async function (opts: { project: string }) {
...
program
  ...
  .action(async function (opts: { project?: string; query?: string; limit?: string }) {
    await initPolicyStore();
    const policy = await readPolicy();
...
program
  ...
  .option("-p, --project <id>", "project id")
  .option("-l, --limit <n>", "optional limit")
  .action(async function (query: string, opts: { project?: string; limit?: string }) {
    await initPolicyStore();
    const policy = await readPolicy();
    const allowed = policy.projects.filter((p) => p.access !== "deny").map((p) => p.id);
...
program
  ...
  .action(async function () {
    ...
    let manifest: { core_files?: string[] } = {};
    ...
    for (const candidate of manifestCandidates) {
      try {
        ...
        manifest = JSON.parse(raw) as { core_files?: string[] };
        loaded = true;
        break;
      } catch {}
    }
...
expose
  ...
  .action(async function (opts: { port: string }) {
    await initPolicyStore();
...
program.command("serve-mcp").option("-p, --port <port>", "mcp port", "4318").action(async function (opts: { port: string }) {
  await initPolicyStore();
  const port = Number(opts.port);
