...
function earliestIndex(haystack: string, markers: string[]) {
  let best = -1;
...
function inferMode(utterance: string, reasons: string[]): "recall" | "recent" {
...
function inferQuery(utterance: string, reasons: string[]) {
  ...
  if (query !== src) reasons.push("query-normalized");
  return query || src;
}

function inferProjectHint(utterance: string, reasons: string[]) {
  const src = normalizeWhitespace(utterance);
  const srcLow = src.toLowerCase();
...
function inferBuildTopic(utterance: string, reasons: string[]) {
  const src = normalizeWhitespace(utterance);
  const srcLow = src.toLowerCase();
...
export function inferRecallIntent(utterance: string): RecallIntent {
  ...
  let confidence = 0.55;
  if (query.length > 0) confidence += 0.2;
  if (projectHint) confidence += 0.15;
  if (buildTopic) confidence += 0.05;
  if (lower(utterance).includes("chatgpt")) confidence += 0.05;
...
export function correlateWithBuild(
  rows: Array<{ threadId: string; threadTitle: string; projectName: string; excerpt?: string }>,
  buildContext: string
): CorrelationRow[] {
  const buildTokens = tokens(buildContext);
  const correlated: CorrelationRow[] = [];
