import { inferRecallIntent, type RecallIntent } from "./intent.js";
...
function normalizeFromLlm(raw: any): RecallIntent {
  ...
  const reasons = Array.isArray(raw?.reasons) ? raw.reasons.map((r: unknown) => String(r)) : ["llm-intent-parse"];
...
export async function inferRecallIntentWithLlm(utterance: string, opts: LlmOptions = {}): Promise<RecallIntent> {
  const apiKey = opts.apiKey ?? process.env.OPENAI_API_KEY;
  if (!apiKey) return inferRecallIntent(utterance);
  ...
  if (!response.ok) return inferRecallIntent(utterance);

  const payload = (await response.json()) as any;
  const content = payload?.choices?.[0]?.message?.content;
  if (typeof content !== "string" || !content.trim()) return inferRecallIntent(utterance);

  try {
    const parsed = JSON.parse(content);
    const normalized = normalizeFromLlm(parsed);
    if (!normalized.query) return inferRecallIntent(utterance);
    return normalized;
  } catch {
